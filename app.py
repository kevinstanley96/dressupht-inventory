...
st.set_page_config(page_title="Dressupht ERP v2.6", layout="wide")
...
if st.session_state["authentication_status"]:
    ...
    def clean_data(file, loc_col_name="current quantity dressupht pv"):
        ...
        # Extended mapping to include Categories
        mapping = {
            'item name': 'Wig Name', 
        ...
        df['Wig Name'] = df['Wig Name'].astype(str).str.strip()
    ...
    if user_role == 'Admin':
        ...
        if f_pv and st.sidebar.button("🚀 Wipe & Sync to Cloud"):
            ...
            with st.spinner(f"Syncing {total} items with Categories..."):
                for i in range(0, total, 10):
                    ...
                    time.sleep(0.2)
            st.sidebar.success("Database Updated with Categories!")
            st.rerun()
    ...
    with tabs[0]:
        ...
        if not lib_data.empty:
            ...
            st.dataframe(filtered_df[['Category', 'Full Name', 'SKU', 'Stock', 'Price', 'Last_Sync_Date']], use_container_width=True, hide_index=True)
        else:
            st.warning("No data found. Admin must sync.")

...
def get_at_data(table, base_id, headers):
    ...
    try:
        ...
        while True:
            ...
            # Sorting logic: History by Date, Library by Name
...
if st.session_state["authentication_status"]:
    ...
    if curr_user != 'guest':
        ...
        st.sidebar.markdown("---")
        st.sidebar.subheader("⚡ Quick Sync")
        ...
        if f_quick and st.sidebar.button("🚀 Push to Guest Library"):
            with st.spinner("Syncing..."):
                ...
                st.sidebar.success("Updated!"); time.sleep(1); st.rerun()

    st.title("🦱 Dressupht Intelligence")
    search = st.text_input("🔍 Search", placeholder="Try '10 bob' or 'black'...")
    ...
    if curr_user == 'guest':
        ...
        with tabs[0]: # Intake
            ...
            with st.form("in_f", clear_on_submit=True):
                i_sku = st.text_input("Scan SKU").strip(); q = st.number_input("Quantity", 1)
        ...
        with tabs[1]: # Audit
            ...
            with st.form("aud_f", clear_on_submit=True):
                a_sku = st.text_input("Scan SKU").strip(); m = st.number_input("Count Found", 0)
        ...
        with tabs[2]: # Sales
            ...
            else: st.info("Upload THIS and LAST Saturday files to see sales.")

        with tabs[3]: # Compare
            st.subheader("🔄 Haiti vs PV Compare")
            ...
            else: st.info("Upload both Haiti and PV files.")
        ...
        with tabs[5]: # Fast/Slow
            ...
            else: st.info("Upload files to see performance.")

        with tabs[6]: # OOS
            st.subheader("❌ Out of Stock")
            if not df_pv.empty:
                st.dataframe(df_pv[df_pv['Stock'] == 0][['Full Name', 'SKU']], use_container_width=True, hide_index=True)

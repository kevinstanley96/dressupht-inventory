...
if st.session_state["authentication_status"]:
    ...
    try:
        ...
        st.error("Secrets Error. Check AIRTABLE_TOKEN and AIRTABLE_BASE_ID in Streamlit.")
        st.stop()
    ...
    search = st.text_input("🔍 Search Inventory", placeholder="Try '12 bob' or '10 black'...")
    ...
    if curr_user == 'guest':
        ...
        with tabs[0]:
            ...
            if not data.empty:
                ...
                if search:
                    # Multi-word case-insensitive search logic
                    search_terms = search.split()
                    for term in search_terms:
                        view = view[view['Full Name'].str.contains(term, case=False, na=False)]
        ...
        with tabs[1]: # Audit
            with st.form("aud_f", clear_on_submit=True):
                a_sku = st.text_input("Scan SKU").strip(); m = st.number_input("Found Qty", 0)
        ...
        with tabs[2]: # Sales
            ...
            else: st.info("Upload PV files to see sales.")

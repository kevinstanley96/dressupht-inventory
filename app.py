...
# --- 1. HEADER & INTAKE ---
st.title("🦱 Dressupht Pv: Multi-Location Intelligence")
...
file_haiti = col_u3.file_uploader("🌐 Other Location (Dressup Haiti)", type=['xlsx'])
...
if file_pv:
    ...
    m1.metric("💰 PV Total Value", f"${df_pv['Total Value'].sum():,.2f}")
    m2.metric("🔥 PV Sold (Weekly)", int(df_pv['Sold'].sum()))
    m3.metric("⚠️ PV Low Stock", len(df_pv[(df_pv['Stock'] > 0) & (df_pv['Stock'] <= 5)]))
    ...
    if haiti_active:
        ...
        transfer_list = compare_all[compare_all['Request Qty'] > 0].copy()
        m4.metric("🚚 Smart Transfers", len(transfer_list))
    else:
        m4.metric("🌐 Haiti File", "Missing")
    ...
    t1, t2, t3, t4, t5, t6, t7 = st.tabs([
        "🔄 Comparison", "🚚 Smart Transfers", "🔥 Fast/Slow", "❌ OOS", 
    ...
    with t2:
        if haiti_active:
            st.dataframe(get_view(transfer_list[['Full Name', 'SKU', 'Stock_haiti', 'Stock_pv', 'Sold', 'Request Qty']]), use_container_width=True)

    # ... [Skipping middle tabs for brevity, logic remains same] ...
    with t3: st.dataframe(get_view(df_pv[['Full Name', 'SKU', 'Stock', 'Sold']].sort_values('Sold', ascending=False)), use_container_width=True)
    ...
    with t6: st.dataframe(get_view(df_pv[['Full Name', 'SKU', 'Stock', 'Price', 'Total Value']].sort_values('Total Value', ascending=False)), use_container_width=True)

    with t7:
        st.subheader("All Wigs (Multi-Location View)")
        if haiti_active:
            # Merge Haiti stock into the full library view
            full_lib = pd.merge(df_pv, df_haiti[['SKU', 'Stock']], on='SKU', how='left', suffixes=('', '_haiti'))
            full_lib = full_lib.rename(columns={'Stock_haiti': 'Haiti Stock'})
            
            def color_full_library(df):
                # Create a style dataframe (same shape as input)
                styles = pd.DataFrame('', index=df.index, columns=df.columns)
                
                # Apply Purple to Haiti Stock column if PV < 5 and Haiti > 25
                mask = (df['Stock'] < 5) & (df['Haiti Stock'] > 25)
                styles.loc[mask, 'Haiti Stock'] = 'background-color: #9b59b6; color: white'
                
                # Apply standard PV colors (Red for 0, Yellow for <= 5)
                styles.loc[df['Stock'] == 0, 'Stock'] = 'background-color: #ff4b4b; color: white'
                styles.loc[(df['Stock'] > 0) & (df['Stock'] <= 5), 'Stock'] = 'background-color: #ffeb3b; color: black'
                
                return styles

            # Reorder columns to ensure Haiti Stock is last
            cols = [c for c in full_lib.columns if c != 'Haiti Stock'] + ['Haiti Stock']
            st.dataframe(get_view(full_lib[cols]).style.apply(color_full_library, axis=None), use_container_width=True)
        else:

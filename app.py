...
def get_at_data(table, base_id, headers):
    try:
        ...
        # For Master_Inventory, we sort by Name instead of Date
...
if st.session_state["authentication_status"]:
    ...
    try:
        ...
        st.error("Missing Airtable Secrets in Streamlit.")
        st.stop()

    # --- SIDEBAR LOGIC ---
    df_pv = pd.DataFrame()
    if curr_user != 'guest':
        # Standard Admin Uploads
        ...
        # New 3-Hour Quick Sync
        st.sidebar.markdown("---")
        st.sidebar.subheader("⚡ Live Guest Sync")
        st.sidebar.caption("Last update target: 08:10, 11:10, 14:10...")
        ...
        if quick_file:
            ...
            if st.sidebar.button("🚀 Update Guest Library Now"):
                # Placeholder for Airtable Update Logic
                st.sidebar.success(f"Cloud Updated at {datetime.now().strftime('%H:%M')}")
        
        # Load Saturday Data for Analytics
        if file_pv:
            df_pv = clean_data(file_pv, "current quantity dressupht pv")
    
    st.title("🦱 Dressupht Intelligence Center")
    ...
    if curr_user == 'guest':
        ...
        with tabs[0]:
            st.subheader("Available Inventory (PV Depot)")
            # Guest always pulls from Airtable "Master_Inventory"
            cloud_inventory = get_at_data("Master_Inventory", BASE_ID, HEADERS)
            if not cloud_inventory.empty:
                # Security: Drop Price and ID
                guest_view = cloud_inventory.drop(columns=['Price', 'Value', 'id'], errors='ignore')
                if search:
                ...
                st.info("The inventory is being updated. Please check back in a few minutes.")
        ...
        with tabs[0]: # Intake
            ...
            with st.form("in_form", clear_on_submit=True):
                q_i = st.number_input("Qty", min_value=1)
        ...
        with tabs[1]: # Audit
            st.subheader("Physical Audit")
            a_sku = st.text_input("Scan for Audit", key="a_scan").strip()
            # ... Audit Logic ...
        ...
        with tabs[3]: # Full Library
            st.subheader("Internal Library (With Prices)")
            if not df_pv.empty:
                st.dataframe(df_pv, use_container_width=True)
            else:
                st.warning("Upload 'THIS Saturday' file in sidebar to view analytics/prices.")

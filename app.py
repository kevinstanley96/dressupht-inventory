...
st.set_page_config(page_title="Dressupht ERP v2.3", layout="wide")
...
if st.session_state["authentication_status"]:
    ...
    def clean_data(file, loc_col_name="current quantity dressupht pv"):
        ...
        df['Wig Name'] = df['Wig Name'].astype(str).str.strip().replace('nan', 'Unknown Wig')
        df['Style'] = df['Style'].astype(str).str.strip().replace('nan', '')
        df['Full Name'] = df['Wig Name'] + " (" + df['Style'] + ")"
        
        # Safe Numeric Conversion (The Fix for ValueError)
        if loc_col_name in df.columns:
            ...
            if possible_stock_cols:
                df['Stock'] = pd.to_numeric(df[possible_stock_cols[0]], errors='coerce').fillna(0).astype(int)
    ...
    with tabs[0]:
        ...
        if not lib_data.empty:
            ...
            st.warning("Library is empty. Admin must sync a Square Export first.")
    ...
    if user_role in ['Admin', 'Manager']:
        with tabs[2]:
            ...
            if a_sku and not lib_data.empty:
                ...
                if not match.empty: 
                    st.success(f"Wig Identified: {match.iloc[0]['Full Name']}")

            with st.form("audit_v2_3"):
                c1, c2 = st.columns(2)
                ...
                if st.form_submit_button("Submit Audit"):
                    ...
                    st.success(f"Audit Logged! Physical: {total} | System: {sys_s} | Diff: {int(total-sys_s)}")

    # --- TAB 4: COMPARE ---
    if user_role == 'Admin':
        with tabs[3]:
            st.subheader("🔄 Haiti vs. PV Comparison")
            f_haiti = st.file_uploader("Upload Dressup Haiti Export", type=['xlsx'], key="haiti_u")
            if f_haiti and not lib_data.empty:
                df_h = clean_data(f_haiti, "current quantity dressup haiti")
                comp = pd.merge(lib_data[['SKU', 'Full Name', 'Stock']], df_h[['SKU', 'Stock']], on='SKU', how='inner', suffixes=('_PV', '_Haiti'))
    ...
    if user_role == 'Admin':
        with tabs[6]:
            ...
            if st.button("📤 Backup to GitHub"):
                ...
                if res.status_code in [200, 201]: st.success("Backup Successful!")

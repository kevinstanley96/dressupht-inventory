...
if st.session_state["authentication_status"]:
    ...
    try:
        ...
        BASE_ID = st.secrets["BASEID"] if "BASEID" in st.secrets else st.secrets["AIRTABLE_BASE_ID"]
        HEADERS = {"Authorization": f"Bearer {AIR_TOKEN}", "Content-Type": "application/json"}
    except:
        st.error("Secrets Error. Check Streamlit settings."); st.stop()
    ...
    search = st.text_input("🔍 Search", placeholder="Wig name, length, or SKU...")
    ...
    if curr_user == 'guest':
        ...
        with tabs[0]: # Intake
            with st.form("in_f", clear_on_submit=True):
                i_sku = st.text_input("Scan SKU").strip(); q = st.number_input("Qty Received", 1)
        ...
        with tabs[1]: # RESTORED AUDIT FEATURES
            ...
            with st.form("aud_f", clear_on_submit=True):
                ...
                with col1:
                    ...
                    m_stock = st.number_input("Stock in Box/Storage", min_value=0, step=1)
                    m_mannequin = st.number_input("Exposed (Mannequin)", min_value=0, step=1)
                with col2:
                    m_counter = st.selectbox("Who is counting?", ["Kevin", "Staff 1", "Staff 2"])
                    m_returns = st.number_input("Returns/Defects", min_value=0, step=1)
                
                if st.form_submit_button("Log Full Audit", use_container_width=True):
                    if a_sku:
                        total_found = m_stock + m_mannequin
                        requests.post(f"https://api.airtable.com/v0/{BASE_ID}/Inventory_Audit", headers=HEADERS, json={"records": [{"fields": {
                            "Date": datetime.utcnow().isoformat(),
                            "SKU": a_sku,
                            "Manual_Qty": total_found,
                            "Exposed_Qty": m_mannequin,
                            "Returns": m_returns,
                            "Counter_Name": m_counter,
                            "User": curr_user
                        }}]})
                        st.success("Audit Recorded!"); time.sleep(0.5); st.rerun()
            
            auds = get_at_data("Inventory_Audit", BASE_ID, HEADERS)
            if not auds.empty:
                # Showing the detailed columns in the history table
                st.dataframe(auds[['Date', 'Time', 'SKU', 'Manual_Qty', 'Exposed_Qty', 'Returns', 'Counter_Name']], use_container_width=True, hide_index=True)
        ...
        with tabs[3]: # Compare
            if haiti_ready and not df_pv.empty:
                st.dataframe(pd.merge(df_haiti[['SKU', 'Stock']], df_pv[['SKU', 'Full Name', 'Stock']], on='SKU', suffixes=('_haiti', '_pv')), use_container_width=True, hide_index=True)
